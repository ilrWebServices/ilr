...
class EventListing extends ParagraphsBehaviorBase {
    ...
    if (array_key_exists('_localist', $sources) && $localist_data = $this->getLocalistEvents($keywords, $daterange_start, $daterange_end, $events_shown)) {
      foreach ($localist_data['events'] as $localist_event) {
        $events[] = new IlrEvent(
          ...
          $this->getSeriesName($localist_event),
    ...
    foreach ($events as $event) {
      ...
      // This is a localist event, so use a custom theme template.
        ...
        if (!empty($event->object['photo_url'])) {
          ...
          $event->object['series'] = $event->series;
        }
  ...
  /**
   * Encapsulate logic here for localist series names.
   */
  private function getSeriesName($localist_event) {
    $series = FALSE;

    foreach ($localist_event['event']['keywords'] as $keyword) {
      if (strpos($keyword,'Series:') !== FALSE) {
        $series = trim(str_replace('Series:', '', $keyword));
        break;
      }
    }
    return $series;
